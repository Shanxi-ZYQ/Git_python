import random
import os
import time

def main():
    ALL_DIGITS = '0123456789'
    MAX_TRY = 6
    NUM_DIGITS = 3

    os.system('cls')
    target = ''.join(random.sample(ALL_DIGITS,k=NUM_DIGITS)) 
    print(target)

    round = 1
    start_time = int(time.time())
    while 1:
        while 1:
            guess = input(f'请猜一个{NUM_DIGITS}位不重复的数字:')
            if len(guess) == 3 and len(guess) == len(set(guess)) and guess.isdigit():
                break
            else:
                print(f'请输入{NUM_DIGITS}位无重复数字')

        if guess == target:
            end_time = int(time.time())
            print(f'恭喜你猜对了,总共用时{end_time-start_time}秒')
            break
        else:
            print(get_glue(target, guess))

        if round >= MAX_TRY:
            print(f'您已经猜了{round}次,挑战失败')
            break
        round += 1



def get_glue(target, guess):
    glue = ''
    for index in range(len(guess)):
        if guess[index] == target[index]:
            glue += '✅'
        elif guess[index] in target:
            glue += '⚪'
        else:
            glue += '❌'
    glue = ''.join(sorted(list(glue)))
    return glue

main()